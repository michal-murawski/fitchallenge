...
const SignUpSignInComponent = ({
  ...
  >
    <Paragraph size="$5" fontWeight="700" opacity={0.8} marginBottom="$1">{type === "sign-up" ? "Create your account" : "Sign in to your account"}</Paragraph>
    {
    ...
    <XStack alignItems="center" width="100%" justifyContent="space-between">
      ...
      <Paragraph size="$3" opacity={0.5}>or</Paragraph>
      <Stack height="$0.25" backgroundColor="black" width="$10" opacity={0.1} />
    </XStack>
    {
    ...
    >{type === "sign-up" ? "Sign up" : "Sign in"}</Button>
    {
      /* or sign in, in small and less opaque font */
    }
    <XStack>
      <Paragraph size="$2" marginRight="$2" opacity={0.4}>{type === "sign-up" ? "Already have an account?" : "Don\u2019t have an account?"}</Paragraph>
      <Link href={type === "sign-up" ? "/sign-in" : "/sign-up"}><Paragraph
        cursor="pointer"
        size="$2"
        fontWeight="700"
        opacity={0.5}
        hoverStyle={{ opacity: 0.4 }}
      >{type === "sign-up" ? "Sign in" : "Sign up"}</Paragraph></Link>
    </XStack>
    {
      /* forgot password */
    }
    {type === "sign-in" && <XStack marginTop="$-2.5">
      <Paragraph size="$2" marginRight="$2" opacity={0.4}>Forgot your password?</Paragraph>
      <Link href="/password-reset"><Paragraph
        cursor="pointer"
        size="$2"
        fontWeight="700"
        opacity={0.5}
        hoverStyle={{ opacity: 0.4 }}
      >Reset it</Paragraph></Link>
    </XStack>}
  </YStack>;
};
export {
